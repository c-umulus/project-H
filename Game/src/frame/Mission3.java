...
public class Mission3 extends JLabel{ // 
	...
	protected int left; // 누르면 1, 안누르면 0
    private final Object leftLock = new Object(); // 왼쪽키 동기화를 위한 객체

	
	
	public Mission3 (PlayerPlane player, int x, int y, int width, int height) {
		this.player = player;
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
		this.image = new ImageIcon("images/mission3.png").getImage();
	...
	public void setLeft(int left) { //left 입력 확인
        synchronized (leftLock) {
            this.left = left;
        }
	}
