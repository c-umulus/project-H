...
public class Enemy10 extends EnemyUnit {

	private Enemy10 enemy10 = this;
	private static final String TAG = "Enemy10 : ";

	ArrayList<EnemyAttack> enemyAttackkList = new ArrayList<EnemyAttack>();
	private EnemyAttack enemyAttack;

	public Enemy10(PlayerPlane player, int x, int y, int w, int h) {
		this.player = player;
		this.x = x;
		this.y = y;
		this.width = w;
		this.height = h;
		this.image = new ImageIcon("images/enemy10.png").getImage();
		this.life = 4;
		this.crushCheck = false;
		this.islife = true;

		this.player.contextAdd(enemy10);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (!player.getInvincible() && y < 900 && y > -300) {
					...
					try {
						if (collision) {
							explosePlayer(player, enemy10); // 충돌 폭발 메서드
						}
						Thread.sleep(10);

						if (crushCheck) {
							explosePlayer(enemy10);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	private void bulletCreate() {
		if (count % 100 == 0) {
			enemyAttack = new EnemyAttack(enemy10, player, x + 20, y + 40, 270, 2, 50, 50);
			enemyAttackkList.add(enemyAttack);
			enemyAttack = new EnemyAttack(enemy10, player, x + 140, y + 40, 270, 2, 50, 50);
			enemyAttackkList.add(enemyAttack);
