...
public class Enemy9 extends EnemyUnit {

    private Enemy9 enemy9 = this;
    private static final String TAG = "Enemy9 : ";

    ArrayList<EnemyAttack> enemyAttackList = new ArrayList<EnemyAttack>();
    private EnemyAttack enemyAttack;

    public Enemy9(PlayerPlane player, int x, int y, int w, int h) {
        this.player = player;
        this.x = x;
        this.y = y;
        this.width = w;
        this.height = h;
        this.image = new ImageIcon("images/enemy9.png").getImage();
        this.life = 3;
        this.crushCheck = false;
        this.islife = true;

        this.player.contextAdd(enemy9);
            ...
            public void run() {
                ...
                while (islife) {
                    try {
                        ...
                        if (y > 900) {
                            // System.out.println("enemy9 thread terminated");
                            islife = false;
                        }
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (!player.getInvincible() && y < 900 && y > -300) {
					...
					try {
						if (collision) {
							explosePlayer(player, enemy9); // 충돌 폭발 메서드
						}
						Thread.sleep(10);

						if (crushCheck) {
							explosePlayer(enemy9);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
    ...
    private void bulletCreate() {
        if (count % 120 == 0) {
            enemyAttack = new EnemyAttack(enemy9, player, x + 20, y + 40, 300, 2, 30, 30);
            enemyAttackList.add(enemyAttack);

            enemyAttack = new EnemyAttack(enemy9, player, x + 80, y + 40, 1300, 2, 30, 30);
            enemyAttackList.add(enemyAttack);
        }
    }
